export const nodeTags = [
    {
        name: 'BODY',
        icon: '<svg data-wf-icon="ElementBodyIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M3 2C2.44772 2 2 2.44772 2 3V13C2 13.5523 2.44772 14 3 14H13C13.5523 14 14 13.5523 14 13V3C14 2.44772 13.5523 2 13 2H3ZM3 5V13H13V5H3Z" fill="currentColor"></path></svg>'
    },
    
    {
        name: 'SECTION',
        icon: '<svg data-wf-icon="ElementSectionIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 13C2 13.5523 2.44772 14 3 14L13 14C13.5523 14 14 13.5523 14 13L14 3C14 2.44771 13.5523 2 13 2L3 2C2.44771 2 2 2.44772 2 3L2 13ZM3 4L3 3L13 3L13 4L3 4ZM3 5L3 11L13 11L13 5L3 5ZM13 12L3 12L3 13L13 13L13 12Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'DIV',
        icon: '<svg data-wf-icon="ElementDivIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 3C2 2.44772 2.44772 2 3 2H13C13.5523 2 14 2.44772 14 3V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V3ZM13 3L3 3V13H13V3Z" fill="currentColor"></path></svg>'
    },

    {
        name: 'MAIN',
        icon: '<svg data-wf-icon="ElementDivIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 3C2 2.44772 2.44772 2 3 2H13C13.5523 2 14 2.44772 14 3V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V3ZM13 3L3 3V13H13V3Z" fill="currentColor"></path></svg>'
    },

    {
        name: 'NAV',
        icon: '<svg data-wf-icon="HomeIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8.35359 1.64645C8.15833 1.45118 7.84175 1.45118 7.64648 1.64645L1.14648 8.14645L1.85359 8.85355L3.00004 7.70711V12C3.00004 12.5523 3.44775 13 4.00004 13H12C12.5523 13 13 12.5523 13 12V7.70711L14.1465 8.85355L14.8536 8.14645L8.35359 1.64645ZM12 6.70711L8.00004 2.70711L4.00004 6.70711V12H12V6.70711Z" fill="currentColor"></path></svg>'
    },
    
    {
        name: 'IMG',
        icon: '<svg data-wf-icon="ElementImageIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 7C6.55228 7 7 6.55228 7 6C7 5.44772 6.55228 5 6 5C5.44772 5 5 5.44772 5 6C5 6.55228 5.44772 7 6 7Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M2 3C2 2.44772 2.44772 2 3 2H13C13.5523 2 14 2.44772 14 3V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V3ZM13 3L3 3V12.2929L8 7.29289L13 12.2929V3ZM8 8.70711L12.2929 13H3.70711L8 8.70711Z" fill="currentColor"></path></svg>'
    },

    {
        name: 'CUSTOM',
        icon: '<svg data-wf-icon="ElementEmbedIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 3C2 2.44772 2.44772 2 3 2H13C13.5523 2 14 2.44772 14 3V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V3ZM3 13V3H13V13H3ZM8.26565 4.5H9.29004L7.73458 11.5H6.71018L8.26565 4.5ZM5.20711 8L6.35355 6.85355L5.64645 6.14645L4.14645 7.64645L3.79289 8L4.14645 8.35355L5.64645 9.85355L6.35355 9.14645L5.20711 8ZM10.3536 6.14645L11.8536 7.64645L12.2071 8L11.8536 8.35355L10.3536 9.85355L9.64645 9.14645L10.7929 8L9.64645 6.85355L10.3536 6.14645Z" fill="currentColor"></path></svg>'
    },

    // Typography
    {
        name: 'H1',
        icon: '<svg data-wf-icon="ElementH1Icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 12V4H4V8H7V4H8V12H7V9H4V12H3Z" fill="currentColor"></path><path d="M12 4C12 4.55228 11.5523 5 11 5H10.5V6H11C11.3643 6 11.7058 5.90261 12 5.73244V12H13V4H12Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'H2',
        icon: '<svg data-wf-icon="ElementH2Icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 4V12H4V9H7V12H8V4H7V8H4V4H3Z" fill="currentColor"></path><path d="M10 7V5.5C10 4.67157 10.6716 4 11.5 4H12.5C13.328 4 13.9993 4.67092 14 5.49879V7.12546C13.9997 7.48837 13.8679 7.83888 13.6289 8.11202L11.1019 11H14V12H10V10.7407L12.8763 7.45352C12.956 7.36237 13 7.24538 13 7.12426V5.5C13 5.22386 12.7761 5 12.5 5H11.5C11.2239 5 11 5.22386 11 5.5V7H10Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'H3',
        icon: '<svg data-wf-icon="ElementH3Icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 12V4H4V8H7V4H8V12H7V9H4V12H3Z" fill="currentColor"></path><path d="M12.5 5H10V4H12.5C13.3284 4 14 4.67157 14 5.5V6.5C14 6.88418 13.8556 7.23462 13.6181 7.5C13.8556 7.76538 14 8.11582 14 8.5V10.5C14 11.3284 13.3284 12 12.5 12H10V11H12.5C12.7761 11 13 10.7761 13 10.5V8.5C13 8.22386 12.7761 8 12.5 8H11V7H12.5C12.7761 7 13 6.77614 13 6.5V5.5C13 5.22386 12.7761 5 12.5 5Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'H4',
        icon: '<svg data-wf-icon="ElementH4Icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 4V12H4V9H7V12H8V4H7V8H4V4H3Z" fill="currentColor"></path><path d="M10 4V6.5C10 7.32843 10.6716 8 11.5 8H13V12H14V4H13V7H11.5C11.2239 7 11 6.77614 11 6.5V4H10Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'H5',
        icon: '<svg data-wf-icon="ElementH5Icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 4V12H4V9H7V12H8V4H7V8H4V4H3Z" fill="currentColor"></path><path d="M10 4V8H12.5C12.7761 8 13 8.22386 13 8.5V10.5C13 10.7761 12.7761 11 12.5 11H10V12H12.5C13.3284 12 14 11.3284 14 10.5V8.5C14 7.67157 13.3284 7 12.5 7H11V5H14V4H10Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'P',
        icon: '<svg data-wf-icon="ElementPIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M5 4H9C10.1046 4 11 4.89543 11 6V7C11 8.10457 10.1046 9 9 9H6V12H5V4ZM6 8H9C9.55228 8 10 7.55228 10 7V6C10 5.44772 9.55228 5 9 5H6V8Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'A',
        icon: '<svg data-wf-icon="ElementLinkIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8.64643 3.64667C9.67012 2.62298 11.3298 2.62298 12.3535 3.64667C13.3772 4.67036 13.3772 6.33009 12.3535 7.35378L11.3535 8.35378L10.6464 7.64667L11.6464 6.64667C12.2796 6.01351 12.2796 4.98695 11.6464 4.35378C11.0133 3.72062 9.9867 3.72061 9.35354 4.35378L8.35354 5.35378L7.64643 4.64667L8.64643 3.64667ZM6.35354 7.35378L5.35354 8.35378C4.72037 8.98695 4.72037 10.0135 5.35354 10.6467C5.9867 11.2798 7.01326 11.2798 7.64643 10.6467L8.64643 9.64667L9.35354 10.3538L8.35353 11.3538C7.32984 12.3775 5.67012 12.3775 4.64643 11.3538C3.62274 10.3301 3.62274 8.67036 4.64643 7.64667L5.64643 6.64667L6.35354 7.35378ZM7.35354 9.35378L10.3535 6.35378L9.64643 5.64667L6.64643 8.64667L7.35354 9.35378Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'VIDEO',
        icon: '<svg data-wf-icon="ElementVideoIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M4 3.41905C4 2.63812 4.8551 2.15855 5.52145 2.56577L13.0626 7.17424C13.7007 7.56417 13.7007 8.49087 13.0626 8.8808L5.52145 13.4893C4.8551 13.8965 4 13.4169 4 12.636V3.41905ZM12.5411 8.02752L5 3.41905V12.636L12.5411 8.02752Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'SOURCE',
        icon: '<svg data-wf-icon="ElementVideoIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M4 3.41905C4 2.63812 4.8551 2.15855 5.52145 2.56577L13.0626 7.17424C13.7007 7.56417 13.7007 8.49087 13.0626 8.8808L5.52145 13.4893C4.8551 13.8965 4 13.4169 4 12.636V3.41905ZM12.5411 8.02752L5 3.41905V12.636L12.5411 8.02752Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'FORM',
        icon: '<svg data-wf-icon="ElementFormBlockIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 5H2V4H14V5Z" fill="currentColor"></path><path d="M14 8H2V7H14V8Z" fill="currentColor"></path><path d="M2.5 11C2.22386 11 2 11.2239 2 11.5V12.5C2 12.7761 2.22386 13 2.5 13H7.5C7.77614 13 8 12.7761 8 12.5V11.5C8 11.2239 7.77614 11 7.5 11H2.5Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'SPAN',
        icon: '<svg data-wf-icon="ElementSpanIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 5H11.5V4H4.5V5H7.5V12H8.5V5Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'INPUT',
        icon: '<svg data-wf-icon="ElementInputIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 4C2 3.44772 2.44771 3 3 3H13C13.5523 3 14 3.44772 14 4V12C14 12.5523 13.5523 13 13 13H3C2.44772 13 2 12.5523 2 12V4ZM13 4L3 4V12H13V4Z" fill="currentColor"></path><path opacity="0.6" fill-rule="evenodd" clip-rule="evenodd" d="M4 11V5H5V11H4Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'BUTTON',
        icon: '<svg data-wf-icon="ElementButtonIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 3H7.5V7.00001L11 7C11.5523 7 12 7.44771 12 8V10.5858C12 10.851 11.8946 11.1054 11.7071 11.2929L10.2929 12.7071C10.1054 12.8946 9.851 13 9.58579 13L6.1894 13.0001L2.96973 9.78039L4.03039 8.71973L6 10.6893V3Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'UL',
        icon: '<svg data-wf-icon="ElementListDefaultIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.5 4.5C4.05228 4.5 4.5 4.05228 4.5 3.5C4.5 2.94772 4.05228 2.5 3.5 2.5C2.94772 2.5 2.5 2.94772 2.5 3.5C2.5 4.05228 2.94772 4.5 3.5 4.5Z" fill="currentColor"></path><path d="M6 4H14V3H6V4Z" fill="currentColor"></path><path d="M6 8H14V7H6V8Z" fill="currentColor"></path><path d="M6 12H14V11H6V12Z" fill="currentColor"></path><path d="M4.5 7.5C4.5 8.05228 4.05228 8.5 3.5 8.5C2.94772 8.5 2.5 8.05228 2.5 7.5C2.5 6.94772 2.94772 6.5 3.5 6.5C4.05228 6.5 4.5 6.94772 4.5 7.5Z" fill="currentColor"></path><path d="M3.5 12.5C4.05228 12.5 4.5 12.0523 4.5 11.5C4.5 10.9477 4.05228 10.5 3.5 10.5C2.94772 10.5 2.5 10.9477 2.5 11.5C2.5 12.0523 2.94772 12.5 3.5 12.5Z" fill="currentColor"></path></svg>'
    },
    {
        name: 'LI',
        icon: '<svg data-wf-icon="ElementListItemIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M6 3.5H14" stroke="currentColor"></path><path d="M6 7.5H14" stroke="currentColor"></path><path opacity="0.4" d="M6 11.5H14" stroke="currentColor"></path><rect opacity="0.4" x="2.5" y="10.5" width="2" height="2" rx="0.5" fill="currentColor"></rect><rect x="2.5" y="6.5" width="2" height="2" rx="0.5" fill="currentColor"></rect><rect opacity="0.4" x="2.5" y="2.5" width="2" height="2" rx="0.5" fill="currentColor"></rect></svg>'
    },

    // FORM
    {
        name: 'LABEL',
        icon: '<svg data-wf-icon="ElementSpanIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 5H11.5V4H4.5V5H7.5V12H8.5V5Z" fill="currentColor"></path></svg>'
    },

    {
        name: 'COMPONENT',
        icon: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" class="s-8d1-x3ARTLd2"><path fill-rule="evenodd" clip-rule="evenodd" d="M8.47885 1.69144C8.18037 1.52863 7.81963 1.52863 7.52115 1.69144L2.52115 4.41871C2.19989 4.59395 2 4.93066 2 5.29661V10.703C2 11.0689 2.19989 11.4056 2.52115 11.5809L7.52115 14.3081C7.81963 14.471 8.18037 14.471 8.47885 14.3081L8.5 14.2966V13.1575V11.5V8.29661L13 5.84206V8H14V5.29661C14 4.93066 13.8001 4.59395 13.4789 4.41871L8.47885 1.69144ZM3.54416 4.99979L8 2.56934L12.4558 4.99979L8 7.43025L3.54416 4.99979ZM3 5.84206L3 10.703L7.5 13.1575V8.29661L3 5.84206ZM13 9V11H15V12H13V14H12V12H10V11H12V9H13Z" fill="currentColor" class="s-8d1-x3ARTLd2"></path></svg>'
    }
]