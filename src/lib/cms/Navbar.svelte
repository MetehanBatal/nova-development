<script>
    ...
    function triggerPublish(environment) {
    ...
    function alterBreakpoint() {
        console.log(breakpoints, selectedBreakpointIndex)
        $selectedBreakpoint = breakpoints[selectedBreakpointIndex].breakpointName

        console.log('breakpoint changed: ', $selectedBreakpoint);
    }
...
<div class={`component-navbar ${visible ? 'visible' : ''} ${navbarShrunk ? 'largen' : ''}`}>
    <div>
        ...
        <Dropdown options={$pages.pages.map(({pageName}) => ({name: pageName}))} bind:selectedStatusIndex={$pages.selectedPageIndex} position="left" />
    </div>
