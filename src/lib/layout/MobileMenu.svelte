<script>
    ...
    import Notifications from '../toolkit/Notifications.svelte';
...
<div class="mobile-menu-wrapper">
    <div class="mobile-menu-header">
        <div class="hamburger-icon" class:activated={isMenuOpen} on:click={() => toggleMenu()}>
    ...
    <div class="mobile-menu-items" class:no-padding={activeTab === "Notification"} class:revealed={isMenuOpen}>
        <div class="mobile-menu-tabs-wrapper">
            <div class="mobile-menu-tabs">
                <div class="mobile-menu-tab" class:active={activeTab === "Menu"} on:click={() => activeTab = "Menu"}>
                    Menu
                </div>
                <div class="mobile-menu-tab" class:active={activeTab === "Notification"} on:click={() => activeTab = "Notification"}>
                    Notifications
                </div>
            </div>
        </div>
        {#if activeTab === "Menu"}
            {#each menuItems as menuItem}
                <a href="{menuItem.slug}" on:click={() => isMenuOpen = !isMenuOpen}>
        ...
        <div class="mobile-menu-divider mt-auto"></div>

        <div href="/profile-settings" class="mobile-user-avatar" on:click={() => isMenuOpen = !isMenuOpen}>
            <a href="/profile-settings">
                <div class="mobile-menu-avatar-image">
            ...
            <div class="mobile-menu-item" on:click={() => handleSignOut()}>
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
...
<style>
    ...
    .mobile-menu-tab.active {
        box-shadow: 0 .4rem .4rem -.1rem rgba(0, 0, 0, 0.05);
    ...
    .hamburger-icon {
        display: flex; /* Initially hide on larger screens */
        flex-direction: column;
        align-items: flex-start;
        justify-content: center;
        cursor: pointer;
    ...
    .hamburger-icon.activated .hamburger-line:nth-child(1) {
        transform: rotate(45deg) translate(5px, 5px);
    }

    .hamburger-icon.activated .hamburger-line:nth-child(2) {
        opacity: 0; /* Ortadaki çizgiyi gizle */
    }

    .hamburger-icon.activated .hamburger-line:nth-child(3) {
    ...
    :global(.mobile-menu-item svg path) {
        transition: all .3s ease-in-out;
    }

    :global(.mobile-menu-item:hover svg path ) {
        stroke: white;
    }

    .mobile-menu-items.revealed {
    ...
    :global(.mobile-menu-items .notification-wrapper) {
