...
export async function load({ fetch }) {
    const [pagesRes, componentsRes, stylesRes] = await Promise.all([
        ...
        fetch(`https://preconvert.novus.studio/staging/components/view?limit=10000&offset=0`).then(res => res.json()),
        fetch(`https://preconvert.novus.studio/staging/style/view?limit=10000&offset=0`).then(res => res.json())
    ]);
